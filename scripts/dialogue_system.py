...
class DialogueSystem:
    def __init__(self, tts_system, speech_system=None, realtime_speech=None):
        self.active = False
        self.user_input = ""
        self.tts_system = tts_system  # Store the TTS system instance
        self.speech_system = speech_system  # Store the speech system instance
        self.realtime_speech = realtime_speech  # Store the realtime speech system
        try:
            pygame.font.init()
    ...
    # Add the method to handle real-time Speech-to-Speech
    def handle_realtime_speech(self, start=True):
        """Start or stop a realtime speech-to-speech dialog"""
        if not self.realtime_speech:
            print("[DialogueSystem] No realtime voice system available")
            return False
        
        if start:
            # Select speech based on NPC role
            if self.current_npc == "HR" : 
                success = self.realtime_speech.start_speech_to_speech("Sarah Chen")
            else:
                success = self.realtime_speech.start_speech_to_speech("Michael Chen")

            if success:
                print(f"[DialogueSystem] Started realtime speech with {self.current_npc} voice")
                return True
            else:
                print("[DialogueSystem] Failed to start realtime speech")
                return False
        else:
            # Stop the voice session
            success = self.realtime_speech.stop()
            print(f"[DialogueSystem] Stopped realtime speech: {success}")
            return success
   

    def update_npc_message(self, text):
        """Update NPC message with text from speech API"""
        if text:
