...
from text_to_speech import TextToSpeechSystem
from realtime_speech import RealtimeSpechSystem
from realtime_speech_to_speech import RealtimeSpeechToSpeech
...
class Game3D:
    def __init__(self):
        ...
        self.player = Player()
        self.world = World()
        self.voice_system = SpeechSystem()
                    ...
                    if event.type == pygame.QUIT:
                        ...
                        if keys[pygame.K_LSHIFT] and event.key == pygame.K_q:
                            if self.dialogue.active:
                                ...
                                if self.recording_active:
                                    self.dialogue.handle_realtime_speech(start=False)
                                    self.recording_active = False
                        ...
                        # Shift+T to start real-time voice
                            ...
                            if self.dialogue.active and not self.recording_active:
                                ...
                                success = self.dialogue.handle_realtime_speech(
                                    start=True
                                )
                                print(success)
                                if success:
